...
def read_index(request: Request, db: Session = Depends(database.get_db)):
...
def create_post(content: str = Form(...), author_username: str = Form(...),
                db: Session = Depends(database.get_db)):
    user = crud.get_user_by_username(db, author_username)
    if not user:
        return HTMLResponse("Author not found")
    crud.create_post(db, user.id, content)
    return RedirectResponse(url=f"/profile/{author_username}", status_code=303)
